...
import com.fanap.hcm.core.hcmcore.pcn.services.dto.expression.OneHandOperatorExpression;
...
import com.fanap.hcm.core.hcmcore.pcn.services.impl.MainStatementGeneratorImpl;
...
import static com.fanap.hcm.core.hcmcore.pcn.services.dto.token.TokenType.NEW_LINE;
...
public interface StatementGenerator {
    ...
    default List<Token> selectFirstTokenListToLastNewLine(List<Token> tempTokenList, List<Token> tokenList) {
        if (CollectionUtils.isNotEmpty(tempTokenList)) {
            if (tempTokenList.get(tempTokenList.size() - 1).getTokenType() == NEW_LINE
            ...
            for (int i = tempTokenList.size() - 1; i >= 0; i--) {
                if (tempTokenList.get(i).getTokenType() == NEW_LINE
    ...
    default TwoHandOperatorExpression getOperatorAndTypeOfTwoHandOperatorExpression(Token token) {
        ...
        if (token.getValue().equals("("))
            operatorExpression.setType(PARENTHESIS_EXPRESSION);
        else if (getBinaryList().contains(token.getValue()))
            operatorExpression.setType(BINARY_EXPRESSION);
        else if (getAssignList().contains(token.getValue()))
            operatorExpression.setType(ASSIGNMENT_EXPRESSION);
        else if (getLogicalList().contains(token.getValue()))
            operatorExpression.setType(LOGICAL_EXPRESSION);
    ...
    default OneHandOperatorExpression getOperatorAndTypeOfOneHandOperatorExpression(Token token) {
        ...
        if (getUpdateUnaryList().contains(token.getValue()))
            operatorExpression.setType(UPDATE_EXPRESSION);
        else if (getUnaryList().contains(token.getValue()))
            operatorExpression.setType(UNARY_EXPRESSION);
        return operatorExpression;
    }
